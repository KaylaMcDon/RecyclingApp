...
export default function AIScreen() {
    ...
    function predictionFunction() {
        const predict = async (model) => {  
            ...
            const realArray = predictionArray[0];

            const materialArray = ['Aluminium', 'Carton', 'Glass', 'Organic Waste', 'Other Plastics', 'Paper and Cardboard', 'Plastic', 'Textiles', 'Wood'];
            const highestPredictionNum = Math.max(...realArray);
            const numIndex = realArray.indexOf(highestPredictionNum);
            const finalMaterial = materialArray[numIndex];
            
            setAIPrediction("The AI detects it as: "+finalMaterial+" with a strength of: "+Math.round(highestPredictionNum*100)+"%.");

            if (finalMaterial == "Carton" || finalMaterial == "Aluminium" || finalMaterial == "Plastic" || finalMaterial == "Paper and Cardboard" || (GLOBAL.allowGlass && finalMaterial == "Glass")) {
